...
public class RGBBlockColor {
	...
	public static void activateBlocks() {
		...
		if (VersionResolver.isCurrentEqualOrHigher(VersionResolver.ServerVersion.v1_14_R1)) {
			...
			boolean isAbove112 = VersionResolver.isCurrentEqualOrHigher(VersionResolver.ServerVersion.v1_12_R1);
	...
	public static BufferedImage createResizedCopy(BufferedImage originalImage, int scaledHeight,
		...
		BufferedImage scaledBI = new BufferedImage(WIDTH, scaledHeight, imageType);
	...
	public static Pixel[][] convertTo2DWithoutUsingGetRGB(BufferedImage image) {
		if (image.getRaster().getDataBuffer() instanceof DataBufferByte) {
			...
			for (int pixel = 0, row = 0, col = 0; pixel < pixels.length; pixel += pixelLength) {
				...
				int g = 0;
				int a = 255;
				b += ((int) pixels[pixel + offset] & 0xff);
				g += (((int) pixels[pixel + offset + 1] & 0xff));
				r += (((int) pixels[pixel + offset + 2] & 0xff));
...
class RGBBlockValue extends RGBValue {
	...
	private boolean
			hasFaces = false,
			isTop = false,
			hasSameTextureAllSides = true;

	public RGBBlockValue(Color c, String mat) {
		super(c);

		if (Material.matchMaterial(mat) == null) return;
	...
	public RGBBlockValue(Color c, Color c2, Color c3, Color c4, String m) {
		...
		if (Material.matchMaterial(m) == null) return;
	...
	public RGBBlockValue(Color c, String mat, DyeColor d) {
		...
		if (Material.matchMaterial(mat) == null)
			return;
		byte dyecolor;
		if (VersionResolver.isCurrentEqualOrHigher(VersionResolver.ServerVersion.v1_10_R1))
